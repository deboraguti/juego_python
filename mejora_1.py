...
def intentos():
    try:
       ...
       print('Tenías que introducir un número. Vuelve a ejecutar.')
...
def pedir_letra():
    ...
    if len(letras_posibles) == len(letras_no_coincidentes):  #sucede cuando 'letra' no está en la lista  
        print('La letra tiene que estar entre a y z. ¡Has perdido una oportunidad!')
...
def jugar(diccionario): 
    ...
    while len(letras_erroneas) < num_intentos:  #Escogemos el número de intentos
...
def despedida():
    print('Aquí termina el juego. ¡Hasta pronto!')
